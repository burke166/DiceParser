...
public class RerollDiceTests
{
    ...
    public void Four_d6_r1_never_ends_with_a_1()
    {
        ...
        var r = engine.ExecuteWithRng("4d6r1", new QueueRng(0, 5, 0, 5, 0, 5, 0, 5)).Single();
    ...
    public void Four_d6_ro_less_than_3_each_die_rolls_at_most_twice()
    {
        ...
        var r = engine.ExecuteWithRng("4d6ro<3", new QueueRng(1, 0, 1, 0, 1, 0, 1, 0)).Single();
    ...
    public void Four_d6_r_less_than_or_equal_3_all_final_faces_above_3()
    {
        ...
        var r = engine.ExecuteWithRng(
            "4d6r<3",
            new QueueRng(0, 0, 2, 3, 0, 0, 2, 3, 0, 0, 2, 3, 0, 0, 2, 3)).Single();
    ...
    public void Continuous_r_less_than_N_rejected_when_N_not_below_highest_face()
    {
        ...
        var ex6 = Assert.Throws<EvalException>(() => engine.Execute("1d6r<6").Single());
        Assert.Contains("highest face value 6", ex6.Message, StringComparison.OrdinalIgnoreCase);

        var ex7 = Assert.Throws<EvalException>(() => engine.Execute("1d6r<7").Single());
    ...
    {
        var engine = new DiceEngine(0);
        var r = engine.ExecuteWithRng("1d6ro<7", new QueueRng(0, 5)).Single();
    ...
    public void Reroll_applies_before_standard_explode_on_each_physical_roll()
    {
        ...
        var r = engine.ExecuteWithRng("1d6r1!", new QueueRng(0, 5, 5, 1)).Single();
    ...
    {
        var engine = new DiceEngine(0);
        var r = engine.ExecuteWithRng("1d6!r1", new QueueRng(0, 5, 5, 1)).Single();
    ...
    {
        var engine = new DiceEngine(0);
        Assert.Throws<ParseException>(() => engine.Execute("1d6r1r1").Single());
    }
    ...
    {
        var engine = new DiceEngine(0);
        var r = engine.ExecuteWithRng("2d6r=6", new QueueRng(5, 0, 5, 0)).Single();
